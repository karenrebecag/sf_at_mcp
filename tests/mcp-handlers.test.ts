...
describe('MCP tool handlers (fixture mode)', () => {
  ...
  it('soql_query truncates and paginates', async () => {
    ...
    const body2 = parseToolResult(r2);
  ...
  it('describe cache hits on second call', async () => {
    ...
    expect((parseToolResult(r1).data as { cached: boolean }).cached).toBe(false);
    expect((parseToolResult(r2).data as { cached: boolean }).cached).toBe(true);
  });
});
